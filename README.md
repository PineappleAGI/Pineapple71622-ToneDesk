...
|----------|---------------|--------------|------|
| **Gemini (Google)** — default | gemini-2.5-flash | [aistudio.google.com/apikey](https://aistudio.google.com/apikey) | **Free tier** (rate limits apply) |
...
**Free tier varies by model.** Not every Gemini model is free for every project. If you see a quota error with `limit: 0`, that model has no free-tier allowance for your API key/project — try a current Flash model (ToneDesk defaults to `gemini-2.5-flash`), wait for limits to reset, or enable billing. Check live limits at [ai.dev/rate-limit](https://ai.dev/rate-limit).
